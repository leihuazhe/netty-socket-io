...
public class SocketIoServer {
    ...
    private void init() {
        ...
        server.addEventListener(EventType.DATA_DELETE.getName(), String.class,
                (socketIOClient, agentEvent, ackRequest) -> {
                    log.info(" agentEvent: " + agentEvent);
                    server.getRoomOperations("web").sendEvent(EventType.DATA_INSERT.getName(), agentEvent);
                });


        server.addEventListener(EventType.DATA_UPDATE.getName(), String.class,
                (client, data, ackRequest) -> {
                    log.info(" server received getYamlFileResp cmd" + data);
                    server.getRoomOperations("web").sendEvent(EventType.DATA_UPDATE.getName(), data);
                });

        server.addEventListener(EventType.WEB_HELLO.getName(), String.class,
                (client, data, ackRequest) -> {
                    log.info(" server 收到 webHello消息: {}", data);
                    server.getRoomOperations("web").sendEvent(EventType.WEB_HELLO.getName(), "服务端: " + data);
                });
