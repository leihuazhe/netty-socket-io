...
$(document).ready(function () {
    ...
    socket.on(SOC_CONNECT, function () {
        if (socket.connected) {
             ...
             * 注册
             */
            socket.emit(WEB_HELLO, SOCKET_REG_INFO);
    ...
    socket.on(NODE_EVENT, function (data) {
        if (data !== "") {
            console.log("NODE_EVENT", data)
        }
    });

    socket.on(WEB_HELLO, function (data) {
        if (data !== "") {
            console.log("WEB_HELLO from server", data);
            showMessage(SUCCESS, data, "from server");
        }
    });
    /**
     * 获取服务更新时间返回
    ...
    socket.on(SOC_CDISCONNECT, function () {
        showMessage("错误", "已断开与服务器的连接", "断开连接");
    });

    socket.on(CONNECT_ERROR, function () {
        showMessage("错误", "与服务器建立连接失败", "连接失败");
    });

    socket.on(CONNECT_TIMEOUT, function () {
        showMessage("错误", "与服务器建立连接失败", "连接超时");
    });
});
/**
 * 获取已经注册agent列表
...
initSetList = function () {
    var curl = "/api/deploy-sets?sort=name&order=asc";
};
